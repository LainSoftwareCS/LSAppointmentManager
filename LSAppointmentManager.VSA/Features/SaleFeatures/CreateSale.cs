...
public class CreateSale
{
    public class Endpoint : IEndpoint
    {
        ...
        public async Task<IResult> Handler(CreateSaleDto.Request request, CreateSaleDto.Response response, SaleRepository saleRepository, SaleDetailRepository saleDetailRepository, IMapper mapper)
        {
            ...
            var customer = await saleRepository.GetCustomerByIdAsync(request.CustomerId);
            if (customer == null)
            {
            ...
            foreach (var detail in request.Details)
            {
                ...
                var productService = await saleDetailRepository.GetProductServiceByIdAsync(detail.ProductServiceId);
            ...
            decimal subtotal = (saleDetail.Price * saleDetail.Quantity); // Supuesto que tienes un precio en el ProductService
             decimal discount = CalculateDiscount(subtotal); // Lógica de descuento
             decimal taxes = CalculateTaxes(subtotal); // Lógica de impuestos
             decimal total = (subtotal - (taxes + discount));
             ...
             // Mapear la respuesta
             var responses = mapper.Map<CreateSaleDto.Response>(sale);
